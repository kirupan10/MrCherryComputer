...
@endphp
@extends($isAdmin ? 'layouts.admin' : 'layouts.nexora')
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-actions">
                            <a href="{{ shop_route('orders.create') }}" class="btn btn-outline-primary">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-4">
                            <div class="col-12">
                                <div class="alert alert-info">
                                    <h5>🔧 PDF Processing Status</h5>
                                    <div class="row">
                                        <div class="col-md-4">
                                            <strong>PHP Imagick Extension:</strong>
                                            <span class="badge bg-{{ $diagnostics['php_imagick'] ? 'success' : 'danger' }}">
                                                {{ $diagnostics['php_imagick'] ? 'Available' : 'Not Available' }}
                                            </span>
                                        </div>
                                        <div class="col-md-4">
                                            <strong>ImageMagick Binary:</strong>
                                            <span class="badge bg-{{ $diagnostics['imagemagick_binary'] ? 'success' : 'warning' }}">
                                                {{ $diagnostics['imagemagick_binary'] ? 'Found' : 'Not Found' }}
                                            </span>
                                        </div>
                                        <div class="col-md-4">
                                            <strong>Operating System:</strong>
                                            <span class="badge bg-info">{{ $diagnostics['system_info']['os'] }}</span>
                                        </div>
                                    </div>
                                    @if($diagnostics['imagemagick_binary'])
                                        <small class="text-muted">Binary Path: {{ $diagnostics['imagemagick_binary'] }}</small>
                                    @else
                                        <small class="text-warning">PDF preview generation may not work. Consider installing ImageMagick.</small>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <form id="letterheadForm" enctype="multipart/form-data">
                                    @csrf
                                    <div class="mb-3">
                                        <label class="form-label">Select Letterhead File (PDF Only)</label>
                                        <input type="file" class="form-control" name="letterhead" accept=".pdf,application/pdf" required>
                                        <div class="form-text">
                        ...
                        <!-- Sales Record Configuration -->
                        <div class="row mb-4">
                            <div class="col-12">
                                <h4>4. Sales Record Numbering</h4>
                                <p class="text-muted mb-3">Configure invoice/sales record number format. Useful for migrating from existing systems.</p>
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="mb-3">
                                                        ...
                                                        <div class="form-text">
                                                            Prefix for invoice numbers (e.g., "INV", "BILL", "SR")
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="mb-3">
                                                        <label for="invoice_starting_number" class="form-label">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="icon me-1" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                                        ...
                                                        <input type="number" class="form-control" id="invoice_starting_number" name="invoice_starting_number"
                                                               value="{{ $config['invoice_starting_number'] ?? ($nextRecordNumber ?? 1) }}"
                                                               placeholder="1" min="1" step="1">
                                                        <div class="form-text">
                                                            Set the starting number for new invoices. Defaults to the next sequential number based on your existing records.
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="col-12">
                                                    <div class="alert alert-info mb-3">
                                                        ...
                                                        <span id="invoicePreview">
                                                            {{ ($config['invoice_prefix'] ?? 'INV') }}{{ str_pad($config['invoice_starting_number'] ?? 1, 5, '0', STR_PAD_LEFT) }}
                                                        </span>
                                                        <br>
                                                        <small class="text-muted">Next invoice will be numbered as shown above. The system will automatically increment this number for each new sale.</small>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <h4>5. Position Text Elements</h4>
                                ...
                                @if($showImage || isset($config['letterhead_file']))
                                    ...
                                    <div class="alert alert-success mt-2">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <div class="card mt-3">
                                                <div class="card-header">
                                                    <h5>📋 Product Table Configuration</h5>
                                                </div>
                                                <div class="card-body">
                                                    <!-- Table Width Control -->
                                                    <div class="mb-4">
                                                        <label class="form-label"><strong>Table Width</strong></label>
                                                        <input type="number" class="form-control" id="tableWidth"
                                                               value="{{ $config['table_width'] ?? 480 }}"
                                                               min="300" max="570" placeholder="480">
                                                        <small class="text-muted">Width in PDF points (300-570pt). Default: 480pt</small>
                                                        <button type="button" class="btn btn-primary btn-sm mt-2 w-100" id="saveTableWidth">
                                                            <i class="fas fa-save"></i> Save Table Width
                                                        </button>
                                                    </div>
                                            ...
                                            <div class="mt-3">
                                                @if($testOrderId)
                                                <a href="{{ shop_route('orders.download-pdf-bill', $testOrderId) }}" class="btn w-100" target="_blank">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                                        <path d="M14 3v4a1 1 0 0 0 1 1h4"/>
                                                        <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z"/>
                                                ...
                                                <div class="alert alert-warning">
                                                    ...
                                                    <a href="{{ shop_route('orders.create') }}" class="btn btn-sm btn-primary mt-2">Create Order</a>
                                                </div>
                                                @endif
                                            </div>

                                            <!-- PDF Merge Offset Control -->
                                            <div class="mt-4">
                                                <h6 class="mb-3">
                                                    ...
                                                    PDF Position Adjustment
                                                </h6>
                                                <div class="card">
                                                    <div class="card-body">
                                                        <p class="text-muted small mb-3">Adjust how the PDF content is positioned relative to the letterhead background.</p>

                                                        <div class="row">
                                                            <div class="col-6">
                                                                <div class="mb-3">
                                                                    <label class="form-label">X Offset (pixels)</label>
                                                                    <input type="number" class="form-control" id="merge_offset_x"
                                                                           value="{{ $config['merge_offset']['x'] ?? 0 }}"
                                                                           placeholder="0" step="1">
                                                                    <small class="form-text text-muted">Horizontal position (0 = left)</small>
                                                                </div>
                                                            </div>
                                                            <div class="col-6">
                                                                <div class="mb-3">
                                                                    <label class="form-label">Y Offset (pixels)</label>
                                                                    <input type="number" class="form-control" id="merge_offset_y"
                                                                           value="{{ $config['merge_offset']['y'] ?? 0 }}"
                                                                           placeholder="0" step="1">
                                                                    <small class="form-text text-muted">Vertical position (0 = top)</small>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="row">
                                                            <div class="col-12">
                                                                <button id="saveMergeOffset" class="btn btn-primary w-100">
                                                        ...
                                                        <div class="row mt-3">
                                                            <div class="col-12">
                                                                <button id="resetMergeOffset" class="btn btn-outline-secondary w-100">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="16" height="16" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                                                        <path d="M3 12a9 9 0 0 0 9 9c4.418 0 8.018 -2.612 9 -6h-5"/>
                                                                        <path d="M3 12l4 0l0 4"/>
                                                                    </svg>
                                                        ...
                                                        <div class="mt-3 p-3 bg-light rounded" id="offsetStatus" style="display: none;">
                                                            <small id="offsetMessage"></small>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <div class="mt-3">
                                                ...
                                                @if($testOrderId)
                                                <a href="{{ shop_route('orders.download-pdf-bill', $testOrderId) }}" class="btn w-100 mt-2" target="_blank">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                                        <path d="M14 3v4a1 1 0 0 0 1 1h4"/>
                                                        <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z"/>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('letterheadForm').addEventListener('submit', async function(e) {
        e.preventDefault();

        const form = this;
        const formData = new FormData(form);

        // Defensive: ensure we have a CSRF token meta tag and report debug info
        const csrfMeta = document.querySelector('meta[name="csrf-token"]');
        if (!csrfMeta) {
            console.error('CSRF meta tag not found on page');
            showToast('Missing CSRF token. Please refresh the page and try again.', 'error');
            return;
        }

        const csrfToken = csrfMeta.getAttribute('content');
        console.log('DEBUG: Starting letterhead upload at', new Date().toISOString());
        console.log('DEBUG: navigator.onLine =', navigator.onLine);

        // File diagnostics
        const fileInput = form.querySelector('input[type="file"][name="letterhead"]');
        if (fileInput && fileInput.files && fileInput.files.length > 0) {
            const f = fileInput.files[0];
            try {
                console.log('DEBUG: uploading file name=', f.name, 'type=', f.type, 'size=', f.size, 'bytes');
            } catch (err) {
                console.warn('DEBUG: Could not read file details', err);
            }
        } else {
        ...
        try {
            // Quick connectivity ping to site root to see if server responds before attempting large upload
            console.log('DEBUG: Performing connectivity ping (HEAD /)');
            const pingResp = await fetch('/', { method: 'HEAD', cache: 'no-store', credentials: 'same-origin' });
            console.log('DEBUG: Ping response ok=', pingResp.ok, 'status=', pingResp.status);
        } catch (pingErr) {
            console.error('DEBUG: Connectivity ping failed', pingErr);
            showToast('Cannot reach server (connectivity check failed). Verify server is running and reachable.', 'error');
            return;
        }

        // Use AbortController to avoid hanging uploads and give user feedback on timeouts
        const controller = new AbortController();
        const TIMEOUT_MS = 60000; // 60s for debugging (increase only while diagnosing)
        const timeoutId = setTimeout(() => controller.abort(), TIMEOUT_MS);

        // Provide very verbose debug around fetch to capture network errors
        console.log('DEBUG: Sending POST to {{ shop_route("letterhead.upload") }} with CSRF header');

        fetch('{{ shop_route("letterhead.upload") }}', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .then(async response => {
            ...
            // Special-case: session expired / CSRF token mismatch
            if (response.status === 419) {
                showToast('Session expired or CSRF token mismatch. Please refresh the page and login again.', 'error');
                try { const text = await response.text(); console.warn('Server 419 response (first 200 chars):', text.substring(0,200)); } catch (e) {}
                throw new Error('CSRF/session error (419)');
            }

            const contentType = response.headers.get('content-type') || '';
            if (!response.ok) {
                let text = '';
                try { text = await response.text(); } catch (e) { text = response.statusText || 'Server error'; }
                console.error('Upload failed - non-OK response:', response.status, text);
                showToast('Error uploading letterhead: ' + (response.status + ' - ' + (response.statusText || 'Server Error')), 'error');
                // Expose server response trimmed to console for debugging
                console.warn('Server response body (trimmed):', text.substring ? text.substring(0,1000) : text);
                throw new Error('Upload failed: ' + response.status);
            }

            // If content-type indicates JSON, parse it; otherwise show text for debugging
            if (contentType.indexOf('application/json') !== -1) {
                return response.json();
            }

            const text = await response.text();
            console.warn('Expected JSON but got:', text.substring ? text.substring(0,1000) : text);
            showToast('Upload completed but server returned unexpected response. Check server logs.', 'error');
            throw new Error('Unexpected server response');
        })
        .then(data => {
            if (data && data.success) {
                showToast('Letterhead uploaded successfully!', 'success');
                console.log('DEBUG: Upload success payload:', data);
                setTimeout(() => { window.location.reload(); }, 1000);
            } else {
                console.error('DEBUG: Upload response indicates failure:', data);
                showToast('Error uploading letterhead: ' + (data && data.message ? data.message : 'Unknown error'), 'error');
            }
        })
        .catch(error => {
            clearTimeout(timeoutId);
            console.error('DEBUG: Upload error caught:', error);
            console.error('DEBUG: navigator.onLine =', navigator.onLine);
            // Attempt to provide more diagnostic detail for network errors
            if (error.name === 'AbortError') {
                console.error('Upload aborted due to timeout after', TIMEOUT_MS, 'ms');
                showToast('Upload timed out after ' + (TIMEOUT_MS/1000) + ' seconds. Try a smaller file or check server availability.', 'error');
                return;
            }

            // If fetch failed at network layer, browser may give generic TypeError: Failed to fetch
            if (error instanceof TypeError) {
                console.error('TypeError during fetch (likely network error or CORS):', error);
                showToast('Network error while uploading. Check server, CORS, or proxy settings. See console for details.', 'error');
            } else {
                showToast('Error during upload: ' + (error.message || error), 'error');
            }
        });
    });
    ...
    function createDraggableElement(position, editor, container) {
        ...
        const fieldColors = {
            ...
            'customer_address': 'linear-gradient(135deg, rgba(111, 66, 193, 0.95), rgba(89, 53, 154, 0.9))'
        };
    ...
    document.getElementById('savePositions').addEventListener('click', function() {
        ...
        fetch('{{ shop_route("letterhead.save-positions") }}', {
            method: 'POST',
            headers: {
    ...
    document.getElementById('saveToggles')?.addEventListener('click', function() {
        ...
        fetch('{{ shop_route("letterhead.save-toggles") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    // Sales Record Form Handler
    document.getElementById('salesRecordForm')?.addEventListener('submit', function(e) {
        e.preventDefault();
        ...
        fetch('{{ shop_route("letterhead.save-sales-config") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        .then(data => {
            if (data.success) {
                showToast('Sales record settings saved successfully!', 'success');
        ...
        .catch(error => {
            console.error('Error:', error);
            showToast('Error saving sales record settings', 'error');
        });
    });

    // Update invoice preview in real-time
    function updateInvoicePreview() {
        const prefix = document.getElementById('invoice_prefix').value.trim() || 'INV';
        const startNum = parseInt(document.getElementById('invoice_starting_number').value) || 1;
        const paddedNum = String(startNum).padStart(5, '0');
        document.getElementById('invoicePreview').textContent = `${prefix}${paddedNum}`;
    }

    // Add real-time preview update listeners
    document.getElementById('invoice_prefix')?.addEventListener('input', updateInvoicePreview);
    document.getElementById('invoice_starting_number')?.addEventListener('input', updateInvoicePreview);
    ...
    if (regenerateButton) {
        regenerateButton.addEventListener('click', function() {
            ...
            fetch('{{ shop_route("letterhead.regenerate-preview") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    function initializeItemsAlignment() {
        ...
        // Defensive guard: if essential inputs are missing, skip initialization
        if (!startXInput || !endXInput || !widthInput || !calculatedWidthSpan) {
            console.warn('Items alignment controls not present on this page — skipping initialization');
            return;
        }

        // Update width calculation when start or end position changes
        function updateCalculatedWidth() {
            // Re-check inputs in case DOM changed
            const sEl = startXInput;
            const eEl = endXInput;
            const wEl = widthInput;
            const outEl = calculatedWidthSpan;

            if (!sEl || !eEl || !wEl || !outEl) return;

            const startX = parseInt(sEl.value) || 25;
            const endX = parseInt(eEl.value) || 570;
            const width = Math.max(300, endX - startX);

            wEl.value = width;
            outEl.textContent = width;
        }
    ...
    function saveItemsAlignment(startX, endX, width) {
        ...
        fetch('{{ shop_route("letterhead.save-items-alignment") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    // Table Width Save Handler
    const saveTableWidthBtn = document.getElementById('saveTableWidth');
    if (saveTableWidthBtn) {
        saveTableWidthBtn.addEventListener('click', function() {
            const tableWidth = parseInt(document.getElementById('tableWidth').value) || 480;

            if (tableWidth < 300 || tableWidth > 570) {
                showToast('Table width must be between 300 and 570 points', 'error');
                return;
            }

            fetch('{{ shop_route("letterhead.save-table-width") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                },
                body: JSON.stringify({ table_width: tableWidth })
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    showToast('Table width saved successfully: ' + tableWidth + 'pt', 'success');
                } else {
            ...
            .catch(error => {
                console.error('Error:', error);
                showToast('Failed to save table width', 'error');
            });
        });
    }
    ...
    // ============ PDF Merge Offset Handler ============
    const saveMergeOffsetBtn = document.getElementById('saveMergeOffset');
    const resetMergeOffsetBtn = document.getElementById('resetMergeOffset');
    const mergeOffsetXInput = document.getElementById('merge_offset_x');
    const mergeOffsetYInput = document.getElementById('merge_offset_y');
    const offsetStatus = document.getElementById('offsetStatus');
    const offsetMessage = document.getElementById('offsetMessage');

    if (saveMergeOffsetBtn) {
        saveMergeOffsetBtn.addEventListener('click', function() {
            const xValue = parseFloat(mergeOffsetXInput.value) || 0;
            const yValue = parseFloat(mergeOffsetYInput.value) || 0;

            fetch('{{ shop_route("letterhead.save_offset") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            ...
            .then(data => {
                if (data.success) {
                    offsetMessage.textContent = `✅ PDF offset saved successfully! (X: ${xValue}px, Y: ${yValue}px)`;
                    offsetStatus.style.display = 'block';
                    offsetStatus.classList.remove('bg-danger', 'text-danger');
                    offsetStatus.classList.add('bg-light');

                    showToast('PDF Position Saved! Test the PDF to see the changes.', 'success');

                    // Hide message after 5 seconds
                    setTimeout(() => {
                        offsetStatus.style.display = 'none';
                    }, 5000);
                } else {
                    throw new Error(data.message || 'Failed to save offset');
                }
            })
            .catch(error => {
                console.error('Error saving merge offset:', error);
                offsetMessage.textContent = `❌ Error: ${error.message}`;
                offsetStatus.style.display = 'block';
                offsetStatus.classList.add('bg-danger', 'text-danger');
                offsetStatus.classList.remove('bg-light');
            });
        });
    }

    if (resetMergeOffsetBtn) {
        resetMergeOffsetBtn.addEventListener('click', function() {
            mergeOffsetXInput.value = 0;
            mergeOffsetYInput.value = 0;

            if (confirm('Reset PDF position offset to defaults (0, 0)?')) {
                const xValue = 0;
                const yValue = 0;
                ...
                .then(data => {
                    if (data.success) {
                        offsetMessage.textContent = '✅ PDF offset reset to default (0, 0)!';
                        offsetStatus.style.display = 'block';
                        offsetStatus.classList.remove('bg-danger', 'text-danger');
                        offsetStatus.classList.add('bg-light');

                        showToast('PDF Position Reset to Default!', 'success');

                        setTimeout(() => {
                            offsetStatus.style.display = 'none';
                        }, 5000);
                    } else {
                        throw new Error(data.message || 'Failed to reset offset');
                    }
                })
                .catch(error => {
                    console.error('Error resetting merge offset:', error);
                    offsetMessage.textContent = `❌ Error: ${error.message}`;
                    offsetStatus.style.display = 'block';
                    offsetStatus.classList.add('bg-danger', 'text-danger');
                    offsetStatus.classList.remove('bg-light');
