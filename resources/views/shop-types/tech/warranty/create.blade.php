    ...
    <div class="container-fluid">
        <div class="row g-2 align-items-center">
            ...
            <div class="col-auto ms-auto d-print-none">
                <a href="{{ route('warranty-claims.index') }}" class="btn btn-outline-secondary">
...
<div class="page-body">
    <div class="container-fluid">
        <form action="{{ route('warranty-claims.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="product_id" id="product_id" class="form-select @error('product_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($products as $product)
                                            <option value="{{ $product->id }}"
                                                ...
                                                {{ $product->name }}@if($product->code) - {{ $product->code }}@endif (Stock: {{ $product->quantity ?? 0 }})
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    @enderror
                                    <small class="text-muted">Out-of-stock products are included for warranty claims</small>
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="serial_number" class="form-control @error('serial_number') is-invalid @enderror"
                                           value="{{ old('serial_number') }}" placeholder="Enter serial number" required>
                                    @error('serial_number')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <small class="form-hint">Enter the product's serial number</small>
                                </div>
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <textarea name="issue_description" class="form-control @error('issue_description') is-invalid @enderror"
                                              rows="4" placeholder="Describe the issue in detail" required>{{ old('issue_description') }}</textarea>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="sending_method" id="sending_method" class="form-select @error('sending_method') is-invalid @enderror" required>
                                        <option value="courier" {{ old('sending_method', 'courier') == 'courier' ? 'selected' : '' }}>Courier</option>
                                        <option value="handover" {{ old('sending_method') == 'handover' ? 'selected' : '' }}>Handover</option>
                                        <option value="bus" {{ old('sending_method') == 'bus' ? 'selected' : '' }}>Bus</option>
                                    </select>
                                ...
                                <div class="col-md-6 mb-3" id="tracking_number_group">
                                    ...
                                    <input type="text" name="tracking_number" class="form-control @error('tracking_number') is-invalid @enderror"
                                           value="{{ old('tracking_number') }}" placeholder="Enter tracking number">
                ...
                <div class="col-lg-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="resolution_notes" class="form-control @error('resolution_notes') is-invalid @enderror"
                                          rows="4" placeholder="Enter resolution details">{{ old('resolution_notes') }}</textarea>
                    ...
                    <div class="card mt-3">
                        <div class="card-body">
                            ...
                            <a href="{{ route('warranty-claims.index') }}" class="btn btn-outline-secondary w-100 mt-2">
                                Cancel
                            </a>
                        </div>
                    </div>
    ...
    new TomSelect('#product_id', {
        ...
        render: {
            option: function(data, escape) {
                // Parse: "Product Name - CODE123 (Stock: 5)"
                const text = data.text;
                const stockMatch = text.match(/\(Stock:\s*(\d+)\)$/);
                const stock = stockMatch ? stockMatch[1] : '0';
                const textWithoutStock = text.replace(/\s*\(Stock:\s*\d+\)$/, '');
                const parts = textWithoutStock.split(' - ');
                const name = parts[0] || '';
                const code = parts[1] || '';

                const stockClass = parseInt(stock) > 0 ? 'text-success' : 'text-danger';
                ...
                return '<div class="p-2">' +
                    '<div class="d-flex justify-content-between align-items-start">' +
                        ...
                        '<div class="ms-2 text-end">' +
                            '<div class="small ' + stockClass + ' fw-bold">' + stockLabel + '</div>' +
                            '<div class="small text-muted">Qty: ' + escape(stock) + '</div>' +
                        '</div>' +
                    '</div>' +
                    '</div>';
            },
            item: function(data, escape) {
                ...
                return '<div>' +
                    ...
                    ' <small class="text-muted">- Stock: ' + escape(stock) + '</small>' +
                    '</div>';
            }
        }
    });
