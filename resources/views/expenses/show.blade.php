<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @can('expense-edit')
            <a href="{{ route('expenses.edit', $expense) }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">
                Edit Expense
            </a>
            @endcan
        </div>
            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="text-base font-semibold text-gray-900">{{ $expense->expense_date->format('d M, Y') }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-base font-bold text-gray-900">₹{{ number_format($expense->amount, 2) }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-base text-gray-900">{{ $expense->user->name }}</p>
                    </div>
                    @if($expense->approved_by)
                    <div>
                        <label class="block text-sm font-medium text-gray-500 mb-1">Approved By</label>
                        <p class="text-base text-gray-900">{{ $expense->approver->name }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-500 mb-1">Approved At</label>
                        <p class="text-base text-gray-900">{{ $expense->approved_at->format('d M, Y h:i A') }}</p>
                    </div>
                    @endif
