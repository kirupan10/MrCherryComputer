            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @elseif(Auth::user()->isShopOwner())
                        <h4>My Shop</h4>
                        @if($shops->isEmpty())
                            <a href="{{ shop_route('shops.create') }}" class="btn btn-primary">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <a href="{{ shop_route('admin.shops.show', $shop) }}"
                                                       class="btn btn-sm btn-info">
                                                        <i class="fas fa-eye"></i> View
                                                    </a>

                                                    @if(auth()->user()->role === 'shop_owner' && $shop->owner_id === auth()->id())
                                                        <a href="{{ shop_route('admin.shops.edit', $shop) }}"
                                                    ...
                                                    @if(auth()->user()->isInShop($shop->id) && auth()->user()->getActiveShop()?->id !== $shop->id)
                                                        <a href="{{ shop_route('shops.switch', $shop) }}"
                                                    ...
                                                    @if((auth()->user()->role === 'shop_owner' && $shop->owner_id === auth()->id()) || auth()->user()->role === 'admin')
                                                        <form action="{{ shop_route('shops.destroy', $shop) }}"
                                                              method="POST"
                                                              class="d-inline"
