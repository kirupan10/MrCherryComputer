            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <div class="mb-6">
                    <div class="flex justify-between items-center py-3 border-b-2 border-gray-900">
                        ...
                        <span class="text-lg font-bold text-gray-900">LKR
                            {{ number_format($report['total_revenue'] ?? 0, 2) }}</span>
                    </div>
                    <div class="pl-4 space-y-2 mt-2">
                        <div class="flex justify-between py-2">
                            <span class="text-gray-700">Gross Sales</span>
                            <span class="text-gray-900">LKR {{ number_format($report['gross_sales'] ?? 0, 2) }}</span>
                        </div>
                        <div class="flex justify-between py-2">
                            <span class="text-gray-700">Returns</span>
                            <span class="text-red-600">-LKR {{ number_format($report['returns'] ?? 0, 2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    <div class="flex justify-between items-center py-3 border-b border-gray-300">
                        ...
                        <span class="text-base font-semibold text-gray-900">LKR
                            {{ number_format($report['cogs'] ?? 0, 2) }}</span>
                    </div>
                </div>
                ...
                <div class="mb-6 bg-green-50 p-4 rounded">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-lg font-bold text-green-800">LKR
                            {{ number_format($report['gross_profit'] ?? 0, 2) }}</span>
                    </div>
                ...
                <div class="mb-6">
                    <div class="flex justify-between items-center py-3 border-b-2 border-gray-900">
                        ...
                        <span class="text-lg font-bold text-gray-900">LKR
                            {{ number_format($report['total_expenses'] ?? 0, 2) }}</span>
                    </div>
                    <div class="pl-4 space-y-2 mt-2">
                        @foreach($report['expenses_by_category'] ?? [] as $expense)
                            <div class="flex justify-between py-2">
                                <span class="text-gray-700">{{ $expense['category'] }}</span>
                                <span class="text-gray-900">LKR {{ number_format($expense['amount'], 2) }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-{{ ($report['net_profit'] ?? 0) >= 0 ? 'blue' : 'red' }}-50 p-6 rounded-lg">
                    <div class="flex justify-between items-center">
                        <span
                            class="text-2xl font-bold text-{{ ($report['net_profit'] ?? 0) >= 0 ? 'blue' : 'red' }}-800">
                            NET {{ ($report['net_profit'] ?? 0) >= 0 ? 'PROFIT' : 'LOSS' }}
                        </span>
                        <span
                            class="text-2xl font-bold text-{{ ($report['net_profit'] ?? 0) >= 0 ? 'blue' : 'red' }}-800">
                            LKR {{ number_format(abs($report['net_profit'] ?? 0), 2) }}
                        </span>
                    </div>
                ...
                <div class="mt-8 pt-6 border-t border-gray-200">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-gray-50 p-4 rounded">
                            ...
                            <div class="text-xl font-bold text-gray-900">LKR
                                {{ number_format($report['average_sale_value'] ?? 0, 2) }}
                            </div>
                        </div>
                        <div class="bg-gray-50 p-4 rounded">
