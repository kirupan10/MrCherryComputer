    ...
    <div class="container-fluid">
        ...
        <div class="row g-3 mb-4">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <div class="text-muted small">{{ $item->code ?? '-' }}{{ $item->category_name ? ' · ' . $item->category_name : '' }}</div>
                                    </td>
                                    <td class="text-end">{{ number_format($item->total_sold) }}</td>
                                    @if(!Auth::user()->isEmployee())
                                    <td class="text-end">LKR {{ number_format($item->total_revenue, 2) }}</td>
                                    @endif
            ...
            <div class="col-xl-6">
                <div class="card h-100">
                    <div class="card-header">
                        <h3 class="card-title">Top Sellers This Month</h3>
                    ...
                    <div class="table-responsive">
                        <table class="table table-vcenter card-table">
                            ...
                            <tbody>
                                @forelse($mostSellingThisMonth as $item)
                                <tr>
                                    <td>
                                        <div class="fw-medium">{{ $item->name }}</div>
                                        <div class="text-muted small">{{ $item->code ?? '-' }}{{ $item->category_name ? ' · ' . $item->category_name : '' }}</div>
                                ...
                                @empty
                                <tr>
                                    <td colspan="4" class="text-center text-muted py-4">No sales recorded this month.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
        ...
        <div class="row g-3 mb-4">
                ...
                <div class="card h-100">
                    ...
                    <div class="table-responsive">
                        <table class="table table-vcenter card-table">
                            ...
                            <tbody>
                                @forelse($stockByCategory as $item)
                                <tr>
                                    <td>{{ $item->category_name }}</td>
                                    <td class="text-end">{{ $item->product_count }}</td>
                                    <td class="text-end">{{ number_format($item->total_units) }}</td>
                                    @if(!Auth::user()->isEmployee())
                                    <td class="text-end">LKR {{ number_format($item->stock_value, 2) }}</td>
                                    @endif
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="4" class="text-center text-muted py-4">No category data available.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <div class="col-xl-6">
                <div class="card h-100">
                    <div class="card-header">
                        <h3 class="card-title">Slow-Moving Stock</h3>
                        <div class="card-subtitle">No sales in the last 90 days</div>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-vcenter card-table">
                            <thead>
                                <tr>
                                    ...
                                    @if(!Auth::user()->isEmployee())
                                    <th class="text-end">Tied-up Value</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="fw-medium">{{ $item->name }}</div>
                                        <div class="text-muted small">{{ $item->code ?? '-' }}{{ optional($item->category)->name ? ' · ' . optional($item->category)->name : '' }}</div>
                                    </td>
                                    <td class="text-end">{{ $item->quantity }}</td>
                                    @if(!Auth::user()->isEmployee())
                                    <td class="text-end">{{ $item->buying_price !== null ? 'LKR ' . number_format($item->quantity * $item->buying_price, 2) : '-' }}</td>
                                    @endif
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="3" class="text-center text-muted py-4">No slow-moving stock found.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
