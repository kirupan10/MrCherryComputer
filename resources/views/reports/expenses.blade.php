            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div class="bg-white shadow-sm rounded-lg p-6">
                    ...
                    <div class="mt-2 text-3xl font-bold text-gray-900">LKR
                        {{ number_format($summary['total_expenses'] ?? 0, 2) }}</div>
                </div>
                <div class="bg-white shadow-sm rounded-lg p-6">
                    <div class="text-sm font-medium text-gray-500">Approved Expenses</div>
                    <div class="mt-2 text-3xl font-bold text-green-600">LKR
                        {{ number_format($summary['approved_expenses'] ?? 0, 2) }}</div>
                </div>
                <div class="bg-white shadow-sm rounded-lg p-6">
                    <div class="text-sm font-medium text-gray-500">Pending Expenses</div>
                    <div class="mt-2 text-3xl font-bold text-yellow-600">LKR
                        {{ number_format($summary['pending_expenses'] ?? 0, 2) }}</div>
                </div>
            </div>
                        ...
                        @forelse($expensesByCategory as $expense)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-right">
                                    LKR {{ number_format($expense['total'], 2) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                    ...
                    @if($expensesByCategory->count() > 0)
                        <tfoot class="bg-gray-50 font-semibold">
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900 text-right">
                                    LKR {{ number_format($expensesByCategory->sum('total'), 2) }}
                                </td>
